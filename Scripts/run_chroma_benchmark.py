...
Mirrors the other DB workflows and writes artifacts to the results folder.
"""
...
CONFIG = {
    ...
    'output_dir': 'results/chroma_experiment_001',
    'chroma_config': {
        'persist_directory': os.getenv('CHROMA_PERSIST_DIR', 'vector_stores/chroma_db'),
...
def main():
    print("="*70)
    print("Chroma Vector Database Benchmark")
    print("="*70)
    ...
    cc = CONFIG['chroma_config']
    print(f"Persist dir: {cc['persist_directory']} collection={cc['collection_name']}")
    ...
    try:
        ...
        print(f"✅ Collection '{cc['collection_name']}' created")
    except Exception as e:
    ...
    if not results:
        print("\n❌ No successful queries! Check Chroma setup and data.")
        return 1
    ...
    print("\n" + "="*70)
    print("BENCHMARK COMPLETE (Chroma)")
    print("="*70)
    print(f"\nDatabase: chroma")
    print(f"Documents: {num_docs}")
