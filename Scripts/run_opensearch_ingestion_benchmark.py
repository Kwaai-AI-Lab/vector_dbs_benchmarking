#!/usr/bin/env python3
"""
Ingestion performance benchmark for OpenSearch.
Mirrors the ingestion workflow used for pgvector and Chroma.
"""
...
CONFIG = {
    ...
    'output_dir': 'results/opensearch_ingestion_experiment_001',
    'os_config': {
        'host': os.getenv('OPENSEARCH_HOST', 'localhost'),
        'port': int(os.getenv('OPENSEARCH_PORT', 9200)),
        'index_name': 'ingestion_benchmark',
        'auth': ('admin', 'admin'),
        'use_ssl': False,
        'verify_certs': False,
        'engine': 'nmslib',
...
def run_ingestion_experiment(
    ...
    # Recreate index for a clean state
...
def main():
    """Run the full OpenSearch ingestion benchmark."""
    print("="*70)
    print("OpenSearch Ingestion Performance Benchmark")
    print("="*70)
    ...
    all_results = []
    total_experiments = (len(CONFIG['chunk_sizes']) *
                         len(CONFIG['batch_sizes']) *
                         len(CONFIG['doc_counts']) *
                         CONFIG['n_runs'])
    experiment_num = 0
    ...
    if df.empty:
        print("❌ No successful results — check connection or ingestion logic.")
        return 1
    ...
    print("\n" + "="*70)
    print("INGESTION BENCHMARK COMPLETE (OpenSearch)")
    print("="*70)
    print(f"\n📁 Results: {output_dir}")
...
def generate_plots(df: pd.DataFrame, output_dir: Path):
    ...
    ax.set_ylabel('Total Ingestion Time (s)')
    ax.set_title('Ingestion Time vs Chunk Size')
    ax.legend()
    ...
    ax.set_xlabel('Chunk Size (characters)')
    ax.set_ylabel('Throughput (chunks/s)')
    ...
    if not data.empty:
        ...
        ax.bar(['Parsing', 'Embedding', 'Insertion'], values, color=['#2E86AB', '#A23B72', '#F18F01'])
    ...
    ax.plot(data['batch_size'], data['total_time'], marker='o', linewidth=2, color='#2E86AB')
    ...
    data = df[(df['chunk_size'] == 512) & (df['batch_size'] == 100)]
    ax1.plot(data['num_docs'], data['total_time'], marker='o', linewidth=2, color='#2E86AB')
    ax1.set_xlabel('Number of Documents')
    ax1.set_ylabel('Total Ingestion Time (s)')
    ax1.set_title('Scaling with Document Count')
    ax1.grid(True, alpha=0.3)
    ax2.plot(data['num_docs'], data['num_chunks'], marker='s', linewidth=2, color='#A23B72')
    ax2.set_xlabel('Number of Documents')
    ax2.set_ylabel('Number of Chunks')
    ax2.set_title('Chunks vs Documents')
    ...
    ax.set_title('Ingestion Time Heatmap (seconds)')
